        ...
        for (int h = 0; h < mapSize[index][0]; h++) {
            for (int w = 0; w < mapSize[index][1]; w++) {
                ...
                if (cls_max > objectThresh)  // 如果分类分数大于阈值
                {
                    ...
                    {
                        locval = reg[lc * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w];  //获取边界框坐标
...
//    input_tensor = vsi_nn_GetTensor(graph, graph->input.tensors[0]);
    ...
    output1_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output1_tensor);
    output2_tensor = vsi_nn_GetTensor(graph, graph->output.tensors[1]);
    output2_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output2_tensor);
    output3_tensor = vsi_nn_GetTensor(graph, graph->output.tensors[2]);
    output3_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output3_tensor);
    output4_tensor = vsi_nn_GetTensor(graph, graph->output.tensors[3]);
    output4_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output4_tensor);
    output5_tensor = vsi_nn_GetTensor(graph, graph->output.tensors[4]);
    output5_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output5_tensor);
    output6_tensor = vsi_nn_GetTensor(graph, graph->output.tensors[5]);
    output6_buffer = (float *)vsi_nn_ConvertTensorToFloat32Data(graph, output6_tensor);
...
//    // 输出运行时间
//    std::cout << "get_data_runtime: " << duration_1.count() << " ms" << std::endl;
...
//    // 输出运行时间
//    std::cout << "GetConvDetectionResult_runtime: " << duration_2.count() << " ms" << std::endl;
...
//    // 输出运行时间
//    std::cout << "draw_runtime: " << duration_3.count() << " ms" << std::endl;
