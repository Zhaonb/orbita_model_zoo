...
-------------------------------------------*/
/*pre process for lid: attach_input/out0_1*/
vsi_nn_preprocess_source_layout_e source_layout_for_norm_tensor_1 = VSI_NN_SOURCE_LAYOUT_NHWC;
vsi_nn_preprocess_source_format_e source_format_for_norm_tensor_1 = VSI_NN_SOURCE_FORMAT_IMAGE_RGB;
vsi_nn_preprocess_image_size_t size_for_norm_tensor_1 = {224, 224, 3};
vsi_nn_preprocess_image_resize_t resize_for_norm_tensor_1 = {224, 224, 3};
int8_t reverse_channel_for_norm_tensor_1 = 0;
float mean_and_scale_1[] = {131.0912, 103.8827, 91.4953};
vsi_nn_preprocess_mean_and_scale_t mean_and_scale_for_norm_tensor_1 = {mean_and_scale_1, 3, 1.0};
int32_t perm_1[] = {2, 0, 1, 3};
...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.mean[0] = 123.68;
    input_meta_tab[0].image.mean[1] = 116.78;
    input_meta_tab[0].image.mean[2] = 103.94;
...
*/
vsi_status vnn_PreProcessResnetV150Uint8_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessResnetV150Uint8_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessResnetV150Uint8
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
